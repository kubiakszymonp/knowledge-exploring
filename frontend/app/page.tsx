...
const MapPreview = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full h-full bg-stone-200 animate-pulse" /> }
);

const btnPrimary = "w-full sm:w-auto bg-amber-500 hover:bg-amber-600 text-white text-base font-semibold px-6 py-3 rounded-lg transition-colors";
const btnOutline = "w-full sm:w-auto border-2 border-stone-300 hover:border-amber-500 hover:bg-amber-50 text-stone-700 hover:text-amber-700 text-base font-semibold px-6 py-3 rounded-lg transition-colors";
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="w-full flex flex-col items-center px-6 py-12 space-y-16">
        ...
        {/* Map – relative z-0 so header/sidebar (z-50+) stay above Leaflet panes */}
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {places.slice(0, 3).map((place) => (
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-stone-800 group-hover:text-amber-700 transition-colors">{place.name}</h3>
